...
const SelectItem = ({ className, children, ...props }: SelectPrimitive.Item.Props) => {
	return (
		<SelectPrimitive.Item
			...
			className={cn(
				"relative flex w-full cursor-pointer items-center gap-1.5 rounded-md py-1 pr-8 pl-1.5 text-sm outline-hidden select-none hover:bg-[var(--control-hover)] hover:text-foreground focus:bg-[var(--control-hover)] focus:text-foreground data-highlighted:bg-[var(--control-hover)] data-highlighted:text-foreground data-disabled:pointer-events-none data-disabled:cursor-not-allowed data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
				className,
			)}
			{...props}
		>
...
const SelectScrollUpButton = ({ className, ...props }: React.ComponentProps<typeof SelectPrimitive.ScrollUpArrow>) => {
	return (
		<SelectPrimitive.ScrollUpArrow
			...
			className={cn(
				"top-0 z-10 flex w-full cursor-pointer items-center justify-center bg-popover py-1 hover:bg-[var(--control-hover)] [&_svg:not([class*='size-'])]:size-4",
				className,
			)}
			{...props}
		>
...
const SelectScrollDownButton = ({ className, ...props }: React.ComponentProps<typeof SelectPrimitive.ScrollDownArrow>) => {
	return (
		<SelectPrimitive.ScrollDownArrow
			...
			className={cn(
				"bottom-0 z-10 flex w-full cursor-pointer items-center justify-center bg-popover py-1 hover:bg-[var(--control-hover)] [&_svg:not([class*='size-'])]:size-4",
				className,
			)}
			{...props}
		>
