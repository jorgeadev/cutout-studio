...
const JobQueueItem = memo(({ job, onDownload, onDownloadProject, onEdit, onRetry, onRemove }: Omit<JobQueueProps, "jobs"> & { job: JobQueueProps["jobs"][number] }) => {
	return (
		<li className="flex items-center gap-3 py-3">
			<div className="checkerboard size-11 shrink-0 overflow-hidden rounded-lg border border-border">
				<img src={(job.status === "done" ? job.cutoutUrl : job.originalUrl) || "/placeholder.svg"} alt="" loading="lazy" decoding="async" className="h-full w-full object-cover" />
			</div>
...
JobQueueItem.displayName = "JobQueueItem";
...
export const JobQueue = ({ jobs, onDownload, onDownloadProject, onEdit, onRetry, onRemove }: JobQueueProps) => {
	return (
		<ul className="flex max-h-104 flex-col divide-y divide-border overflow-y-auto pr-1">
			{jobs.map((job) => (
				<JobQueueItem key={job.id} job={job} onDownload={onDownload} onDownloadProject={onDownloadProject} onEdit={onEdit} onRetry={onRetry} onRemove={onRemove} />
			))}
		</ul>
	);
};
